...
namespace vgu {
	...
	double dotProduct(const Vector<N>& A, const Vector<N>& B) {
		double sum = 0;
	...
	double calcDetRecursively(const double values[], const int& size);

	template <unsigned char N>
	double det(const Matrix<N, N>& mat) {
		return calcDetRecursively(mat.m_values, N);
	}

	template <unsigned char N = 2>
	double det(const Matrix<2, 2>& mat) {
		return mat(0, 0) * mat(1, 1) - mat(0, 1) * mat(1, 0);
	}

	template <unsigned char N = 3>
	double det(const Matrix<3, 3>& mat) {
		const double firstSum = mat(0, 0) * mat(1, 1) * mat(2, 2) + mat(0, 1) * mat(1, 2) * mat(2, 0) + mat(1, 0) * mat(2, 1) * mat(0, 2);
		const double secondSum = mat(0, 2) * mat(1, 1) * mat(2, 0) + mat(1, 0) * mat(0, 1) * mat(2, 2) + mat(2, 1) * mat(1, 2) * mat(0, 0);
